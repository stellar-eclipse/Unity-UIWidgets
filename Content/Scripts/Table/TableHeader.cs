...
#pragma warning restore 0618
		...
		public virtual void SetCellsSize(IList<float> sizes, bool withListView = true)
		{
			...
			if (IsWidth)
			{
				...
				{
					GetCellInfo(i).SetWidth(sizes[i]);
				}
			}
			else
			{
				for (int i = 0; i < sizes.Count; i++)
				{
					GetCellInfo(i).SetHeight(sizes[i]);
				}
			}
		...
		/// </summary>
		protected bool IsWidth
		{
			get
			{
				return !List.IsHorizontal();
			}
		}
		...
		protected bool PointWithPivot(ref Vector2 point)
		{
			...
			foreach (var cell in CellsInfoOrdered)
			{
				...
				if (CheckLeft(cell.Rect, point))
				{
					...
					if (left != -1)
					{
						...
						leftTargetSize = width ? left_cell.Width : left_cell.Height;
						rightTargetSize = width ? cell.Width : cell.Height;

						sizeLimit = leftTargetSize + rightTargetSize;
						return;
					}
				}
				...
				if (CheckRight(cell.Rect, point))
				{
					...
					if (right != -1)
					{
						...
						leftTargetSize = width ? cell.Width : cell.Height;
						rightTargetSize = width ? right_cell.Width : right_cell.Height;
		...
		public void OnDrag(PointerEventData eventData)
		{
			...
			if (IsWidth)
			{
				if (delta.x > 0)
				{
					leftTarget.preferredWidth = Mathf.Min(leftTargetSize + delta.x, sizeLimit - rightTarget.minWidth);
					rightTarget.preferredWidth = sizeLimit - leftTarget.preferredWidth;
				}
				else
				{
					rightTarget.preferredWidth = Mathf.Min(rightTargetSize - delta.x, sizeLimit - leftTarget.minWidth);
					leftTarget.preferredWidth = sizeLimit - rightTarget.preferredWidth;
				}
			}
			else
			{
				if (delta.y > 0)
				{
					leftTarget.preferredHeight = Mathf.Min(leftTargetSize + delta.y, sizeLimit - rightTarget.minHeight);
					rightTarget.preferredHeight = sizeLimit - leftTarget.preferredHeight;
				}
				else
				{
					rightTarget.preferredHeight = Mathf.Min(rightTargetSize - delta.y, sizeLimit - leftTarget.minHeight);
					leftTarget.preferredHeight = sizeLimit - rightTarget.preferredHeight;
				}
			}
		...
		public virtual void SetColumnWidth(int column, float width, bool resizeRightNeighbour = true)
		{
			...
			var left = GetCellInfo(resizeRightNeighbour ? column : column - 1);
			var right = GetCellInfo(resizeRightNeighbour ? column + 1 : column);
		...
		public virtual void SetColumnHeight(int column, float height, bool resizeBottomNeighbour = true)
		{
			...
			var left = GetCellInfo(resizeBottomNeighbour ? column : column - 1);
			var right = GetCellInfo(resizeBottomNeighbour ? column + 1 : column);
		...
		void ResizeGameObject(GameObject go, int index)
		{
			...
			{
				(go.transform as RectTransform).SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cell.Height);
